...
describe('Stock derives from the ledger in the database — "the Stock never lies"', () => {
  ...
  it('derives Stock as entries minus exits, read from the DB view', () => {
    repo.recordMovement({ productId: 'p-cafe', kind: 'entry', quantity: 10, reason: 'purchase', actorId, at });
    repo.recordMovement({ productId: 'p-cafe', kind: 'exit', quantity: 3, reason: 'sale', actorId, at });
    expect(repo.deriveStock('p-cafe')).toBe(7);
  });

  it('rejects an exit larger than the Stock on hand, leaving the ledger unchanged', () => {
    repo.recordMovement({ productId: 'p-cafe', kind: 'entry', quantity: 3, reason: 'purchase', actorId, at });
    expect(() =>
      repo.recordMovement({ productId: 'p-cafe', kind: 'exit', quantity: 5, reason: 'sale', actorId, at }),
    ).toThrow();
    expect(repo.deriveStock('p-cafe')).toBe(3);
  });

  it('records the actor on each movement and reads it back in the history', () => {
    repo.recordMovement({ productId: 'p-cafe', kind: 'entry', quantity: 5, reason: 'purchase', actorId: 'u-bruno', at });
...
describe('The never-negative guard lives in the schema, so it cannot be bypassed', () => {
  ...
  it('rejects a raw exit that would make the Stock negative, even bypassing the repository', () => {
    ...
    db.prepare("INSERT INTO movements (product_id, kind, quantity, reason, actor_id, at) VALUES ('p-cafe','entry',3,'purchase','u-ana',?)").run(at);
    db.prepare("INSERT INTO movements (product_id, kind, quantity, reason, actor_id, at) VALUES ('p-cafe','exit',3,'sale','u-ana',?)").run(at);
