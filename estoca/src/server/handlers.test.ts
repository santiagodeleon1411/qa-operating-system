...
describe('GET /products honors the contract', () => {
  ...
  it('returns the catalogue with Stock derived from the ledger, in contract shape', () => {
    repo.recordMovement({ productId: 'p-cafe', kind: 'entry', quantity: 10, reason: 'purchase', actorId: actor.id, at });
...
describe('POST /movements honors the contract on both edges', () => {
  ...
  it('records a valid movement, stamps the actor, and echoes it in contract shape', () => {
    const res = postMovement(repo, actor, { productId: 'p-cafe', kind: 'entry', quantity: 4, reason: 'purchase' }, at);
  ...
  it('ignores any `at` the caller tries to send — the server owns the time', () => {
    const res = postMovement(
      repo,
      actor,
      { productId: 'p-cafe', kind: 'entry', quantity: 1, reason: 'purchase', at: '1999-01-01T00:00:00.000Z' },
      at,
    );
    expect(res.status).toBe(201);
  ...
  it('refuses a malformed request at the boundary with the typed error shape', () => {
    const res = postMovement(repo, actor, { productId: 'p-cafe', kind: 'entry', quantity: -3, reason: 'purchase' }, at);
  ...
  it('turns a never-negative refusal into the typed error, not a crash', () => {
    postMovement(repo, actor, { productId: 'p-cafe', kind: 'entry', quantity: 3, reason: 'purchase' }, at);

    const res = postMovement(repo, actor, { productId: 'p-cafe', kind: 'exit', quantity: 5, reason: 'sale' }, at);
...
describe('POST /adjustments reconciles a physical count without ever storing Stock', () => {
  ...
  beforeEach(() => {
    repo = new MovementsRepo(createDb());
    // Coffee starts at 42 in the ledger.
    repo.recordMovement({ productId: 'p-cafe', kind: 'entry', quantity: 42, reason: 'purchase', actorId: actor.id, at });
  });
  ...
  it('records a downward adjustment when the count is below the system', () => {
    const res = count({ productId: 'p-cafe', counted: 39, reason: 'Breakage', expectedStock: 42 });
    expect(res.status).toBe(201);
  ...
  it('records an upward adjustment when the count is above the system', () => {
    const res = count({ productId: 'p-cafe', counted: 50, reason: 'Data entry error', expectedStock: 42 });
    expect(res.status).toBe(201);
  ...
  it('records nothing when the count already matches the system', () => {
    const res = count({ productId: 'p-cafe', counted: 42, reason: 'Breakage', expectedStock: 42 });
    expect(res.status).toBe(200);
  ...
  it('accepts a count of zero as a valid adjustment down to zero', () => {
    const res = count({ productId: 'p-cafe', counted: 0, reason: 'Breakage', expectedStock: 42 });
    expect(res.status).toBe(201);
  ...
  it('refuses a negative count, a non-whole count, and an unknown reason at the boundary', () => {
    for (const bad of [
      { productId: 'p-cafe', counted: -3, reason: 'Breakage', expectedStock: 42 },
      { productId: 'p-cafe', counted: 3.5, reason: 'Breakage', expectedStock: 42 },
      { productId: 'p-cafe', counted: 39, reason: 'nonexistent reason', expectedStock: 42 },
    ]) {
      const res = count(bad);
      expect(res.status).toBe(422);
      expect(() => errorResponse.parse(res.body)).not.toThrow();
  ...
  it('surfaces a Stock change during the count and measures the difference from the count', () => {
    ...
    repo.recordMovement({ productId: 'p-cafe', kind: 'exit', quantity: 1, reason: 'sale', actorId: actor.id, at });

    // First submit: the Stock no longer matches what the count began with. Warn, record nothing.
    const warned = count({ productId: 'p-cafe', counted: 39, reason: 'Breakage', expectedStock: 42 });
    ...
    const confirmed = count({ productId: 'p-cafe', counted: 39, reason: 'Breakage', expectedStock: 42, confirmed: true });
  ...
  it('lets the never-negative trigger refuse an adjustment that would drive Stock below zero', () => {
    ...
    repo.recordMovement({ productId: 'p-cafe', kind: 'exit', quantity: 40, reason: 'sale', actorId: actor.id, at }); // 42 -> 2
    const res = count({ productId: 'p-cafe', counted: 0, reason: 'Breakage', expectedStock: 42, confirmed: true });
    expect(res.status).toBe(422);
...
describe('authorization at the write edge — a role that may not act is refused with 403', () => {
  ...
  it('refuses an entry by the runner (a delivery role) with 403, and nothing reaches the ledger', () => {
    const res = postMovement(repo, runner, { productId: 'p-cafe', kind: 'entry', quantity: 3, reason: 'purchase' }, at);
  ...
  it('allows an exit by the runner — deliveries out are permitted', () => {
    postMovement(repo, actor, { productId: 'p-cafe', kind: 'entry', quantity: 5, reason: 'purchase' }, at);
    const res = postMovement(repo, runner, { productId: 'p-cafe', kind: 'exit', quantity: 2, reason: 'delivery' }, at);
    expect(res.status).toBe(201);
  ...
  it('refuses a theft-or-loss classification by the employee with 403, and nothing reaches the ledger', () => {
    postMovement(repo, actor, { productId: 'p-cafe', kind: 'entry', quantity: 42, reason: 'purchase' }, at);
    const res = postAdjustment(
      repo,
      employee,
      { productId: 'p-cafe', counted: 39, reason: 'Theft or loss', expectedStock: 42 },
      at,
    );
    expect(res.status).toBe(403);
  ...
  it('lets the employee record an unexplained shortfall — recording is separated from classifying', () => {
    ...
    const res = postAdjustment(
      repo,
      employee,
      { productId: 'p-cafe', counted: 39, reason: 'Unclassified shortfall', expectedStock: 42 },
      at,
    );
    expect(res.status).toBe(201);
...
describe('the contract has teeth — a drifted response is caught at the boundary', () => {
  ...
  it('refuses to serve a product view that no longer matches the contract', () => {
    const driftedRepo = {
      listProductViews: () => [
        { id: 'p-cafe', name: 'Ground coffee 500g', threshold: 5, quantity: 10, stockout: false },
      ],
